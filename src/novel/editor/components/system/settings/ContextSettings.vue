  ...
  <div class="setting-page-wrapper custom-scrollbar">
      ...
      <div class="setting-item">
        ...
        <div class="setting-control">
          <div class="flex items-center">
            <input id="needs-preview" type="checkbox" v-model="needsPreview" class="setting-checkbox" />
    ...
    <div class="setting-group mt-4">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-4">
            <div>
              ...
              <div class="flex items-center gap-3">
                <input type="range" min="0" max="10" v-model.number="dynamicContextSettings.prevChapters" class="range-custom flex-1"/>
                <span class="text-sm font-medium w-8 text-center text-[#374151]">{{ dynamicContextSettings.prevChapters }}</span>
              </div>
            ...
            <div>
              ...
              <div class="flex items-center gap-3">
                <input type="range" min="0" max="10" v-model.number="dynamicContextSettings.nextChapters" class="range-custom flex-1"/>
                <span class="text-sm font-medium w-8 text-center text-[#374151]">{{ dynamicContextSettings.nextChapters }}</span>
              </div>
              <p class="setting-description mt-1">自动包含当前章节之后N个章节的大纲内容。</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="setting-item">
        ...
        <div class="setting-control">
          <div class="flex items-center">
            <input id="rag-context" type="checkbox" v-model="isRagEnabled" class="setting-checkbox" />
...
const needsPreview = computed({
  get: () => uiStore.uiState.needsPreview,
  set: (value: boolean) => uiStore.setNeedsPreview(value)
});

const customContextContent = computed({
  get: () => uiStore.uiState.customContextContent,
  set: (value: string) => uiStore.setCustomContextContent(value)
});

const dynamicContextSettings = computed({
  get: () => uiStore.uiState.dynamicContextSettings,
  set: (value) => uiStore.setDynamicContextSettings(value)
});

const isRagEnabled = computed({
  get: () => uiStore.uiState.isRagEnabled,
  set: (value: boolean) => uiStore.setRagEnabled(value)
});
...
.setting-page-wrapper {
  padding: 1rem 2rem 4rem; /* Added bottom padding */
  max-width: 56rem;
  margin: 0 auto;
  overflow-y: auto;
  height: 100%;
