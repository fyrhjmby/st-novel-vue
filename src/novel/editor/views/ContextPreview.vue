// ..\src\novel\editor\views\ContextPreview.vue
  ...
  <div class="min-h-screen p-8 bg-[#F5F5F7]">
    <div class="max-w-[1200px] mx-auto">
      ...
      <div class="mb-8">
        ...
        <!-- 修改：将静态文本绑定到计算属性，以显示动态的任务信息 -->
        <p class="text-[#6B7280]">{{ taskInfoText }}</p>
      </div>
      ...
      <div class="bg-white rounded-xl shadow-lg overflow-hidden border border-gray-100">
        ...
        <div class="h-16 px-8 flex items-center justify-between border-b border-gray-100">
          <button @click="handleCancel" class="text-sm text-[#6B7280] hover:text-[#374151] transition-colors flex items-center gap-2">
...
重点描写方向：卡尔文进入空间站控制室，发现仍在运行的神秘设备，以及他对这些发现的反应和推测。</textarea>
        ...
        <div class="px-8 py-4 bg-[#FAFAFA] border-t border-gray-100 flex items-center justify-between">
          ...
          <div class="flex items-center gap-3">
            <button @click="handleCancel" class="px-4 py-2 text-sm text-[#6B7280] bg-white border border-gray-200 rounded-lg hover:bg-gray-50 transition-colors">
              取消
            </button>
            <button @click="handleExecute" class="px-4 py-2 bg-[#10B981] text-white rounded-lg text-sm font-medium hover:bg-[#059669] transition-colors">
              执行
            </button>
          </div>
        </div>
...
const taskInfoText = computed(() => {
  if (taskForPreview.value) {
    return `即将对《${taskForPreview.value.title}》执行AI任务。检查并确认将要提供给AI的全部信息。`;
  }
  return '在执行AI任务前，检查并确认将要提供给AI的全部信息。';
});

// --- Methods ---
const returnToEditor = () => {
  aiTaskStore.clearTaskForPreview();
  router.push({ name: 'NovelEditorWorkspace' });
}
...
// --- Lifecycle Hooks ---
onMounted(() => {
  // 如果没有待预览的任务，则直接返回编辑器界面，防止用户误入
  if (!taskForPreview.value) {
    console.warn('No task to preview. Redirecting to editor.');
    router.replace({ name: 'NovelEditorWorkspace' });
  }
});
</script>
