...
export const useWorkflowEditorStore = defineStore('workflowEditor', () => {
    ...
    const onDrop = (event: DragEvent) => {
        ...
        const newNode: Node = {
            ...
            type: 'custom',
            position,
            label: name, // This label is for vue-flow internal use, not displayed in custom node
            data: {
                title: name,
    ...
    const loadWorkflow = async () => {
        ...
        try {
            ...
            if (flowData && instance.value) {
                ...
                // Default workflow elements using the new data structure
                elements.value = [
                    { id: '1', type: 'custom', label: '开始', position: { x: 80, y: 80 }, data: { title: '开始', description: '工作流的入口点，定义了启动流程所需的输入参数。', iconName: 'play-circle', iconColor: 'green' } },
